...
class Talk(object):
    ...
    def botslist(self,to,wait,text='',text1='',data=[]):
        to = to
        if data == []:return self.sendMessage(to, "╭───「 BotList 」─\n│BotList: None\n│    | Command |  \n│Add {}\n│  Key:{} add{}:on [@|contact]\n│Del {}\n│  Key:{} del{}:on [@|num|contact]\n╰──────".format(text,text1,text,text,text1,text))
        self.datamention(to,'{}'.format(text),data)

    def adityasuperdata(self,to,wait,text='',text1='',data=[]):
        to = to
        if data == []:return self.sendMessage(to, "╭───「 Blacklist 」─\n│Blacklist: None\n│    | Command |  \n│Add {}\n│  Key:{} add{} [@|contact]\n│Del {}\n│  Key:{} del{} [@|num|contact]\n╰──────".format(text,text1,text,text,text1,text))
        self.datamention(to,'{}'.format(text),data)

    def deletefriendnum(self, to, wait, pesan):
        asd = self.refreshContacts()
        selection = MySplit(self.adityasplittext(pesan,'s'),range(1,len(asd)+1))
        k = len(asd)//20
        d = []
    ...
    def blockfriendnum(self, to, wait, cmd):
        asd = self.refreshContacts()
        ...
        self.sendMessage(to,' 「 Friendlist 」\nWaiting.....')
        for a in range(k+1):
            if a == 0:self.mentionmention(to=to,wait=wait,text='',dataMid=d[:20],pl=-0,ps='╭「 Friendlist 」─\n├ Type: Block Friendlist',pg='BLOCK',pt=d)
            else:self.mentionmention(to=to,wait=wait,text='',dataMid=d[a*20 : (a+1)*20],pl=a*20,ps='├「 Friendlist 」─\n├ Type: Block Friendlist',pg='BLOCK',pt=d)
